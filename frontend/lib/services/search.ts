...
import { getCerebrasClient, getOpenAIClient, getGroqClient } from "@/lib/ai/clients";
import type {
...
export async function detailedWebsiteSummary(
  ...
  try {
    ...
    try {
      ...
      return (
        (finalResponse.choices as ChatCompletion.Choice[])[0].message
          .content || null
      );
    } catch (error) {
...
export async function* getStreamedFinalAnswer(
  request: StreamedFinalAnswerRequest
) {
  const { query, sources, imageSources } = request;
