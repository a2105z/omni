...
import type { AssistantMessage } from "@/lib/schemas/chat";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
export function AiResponseView({
  ...
  const hasBackendSources =
    assistantMessage.backendSources && assistantMessage.backendSources.length > 0;
  const citationSources = assistantMessage.backendSources?.map((s) => ({
    url: s.url,
    title: s.title,
    snippet: s.snippet,
    domain: s.domain,
    ...
    <div className="flex flex-col gap-8">
      ...
      {assistantMessage.error && (
        <div className="rounded-lg border border-destructive/50 bg-destructive/10 p-4 flex flex-col gap-3">
          <p className="text-sm text-destructive">{assistantMessage.error}</p>
          {onRegenerate && (
            <Button
              variant="outline"
              size="sm"
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        {/* Backend: Source panel sidebar */}
        {hasBackendSources && (
          <div className="lg:w-80 flex-shrink-0 order-2 lg:order-1">
            <SourcePanel
              sources={assistantMessage.backendSources!.map((s, i) => ({
                ...s,
                sourceNumber: i + 1,
              }))}
              imageSources={assistantMessage.backendImageSources}
            />
          </div>
        )}

        {/* Search status cards (legacy - hide when backend used) */}
        {!hasBackendSources && (
      ...
      <div className="flex flex-col gap-3">
        {hasBackendSources ? (
          ...
          <div className="prose dark:prose-invert max-w-none [&>*]:my-5 [&_p]:leading-relaxed [&_a]:inline-flex [&_a]:items-center [&_a]:gap-2 [&_a]:rounded [&_a]:bg-secondary [&_a]:px-1 [&_a]:py-0.5 [&_a]:border [&_a]:border-border [&_a]:text-sm [&_a]:text-primary [&_a]:font-bold [&_a]:no-underline hover:[&_a]:bg-card/80 [&_a]:mx-0.5">
            <Markdown
              remarkPlugins={[remarkGfm, remarkMath]}
              rehypePlugins={[rehypeKatex]}
              components={{
        ...
        {onRegenerate && assistantMessage.isDoneGeneratingFinalAnswer && !assistantMessage.error && (
          <Button
            variant="ghost"
            size="sm"
      ...
      {/* Citations (legacy - hide when backend used) */}
      {!hasBackendSources &&
        assistantMessage.processedSearchResults &&
